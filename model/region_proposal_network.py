...
class RegionProposalNetwork(nn.Module):
    ...
    def __init__(
        ...
        # rpn cls
        self.rpn_cls = nn.Conv2d(mid_channels, n_anchor * 2, 1, 1, 0)
        # rpn reg
        self.rpn_reg = nn.Conv2d(mid_channels, n_anchor * 4, 1, 1, 0)
        normal_init(self.conv1, 0, 0.01)
        normal_init(self.rpn_cls, 0, 0.01)
        normal_init(self.rpn_reg, 0, 0.01)
    ...
    def forward(self, x, img_size, scale=1.):
        ...
        * :math:`A` is number of anchors assigned to each pixel.
        ...
        rpn_locs = self.rpn_reg(h)
        # UNNOTE: check whether need contiguous
        # A: Yes
        rpn_locs = rpn_locs.permute(0, 2, 3, 1).contiguous().view(n, -1, 4)

        # rpn基于锚框的正负样本: math:(C, 2*anchor_num, W, H)
